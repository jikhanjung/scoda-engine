...
모바일(세로 모드)에서 뷰 탭 메뉴가 화면에 다 보이지 않는 문제.

- 좁은 화면에서 탭 바를 햄버거 메뉴 또는 드롭다운으로 전환
- 메뉴 아이콘 클릭 시 탭 목록이 세로로 펼쳐지는 구조
- 반응형 breakpoint 설정 (예: 768px 이하)
- 트리 차트, 테이블 등 주요 뷰의 터치 조작 개선 검토

## 우선순위

1. Hub Refresh 버튼 (영향 범위 작음, 운영 즉시 유용)
2. 모바일 반응형 UI (프론트엔드, 사용성 즉시 개선)
3. Animation 동영상 다운로드 (프론트엔드 단독 기능)
4. Admin Backend (설계 검토 필요, 도메인 의존성 높음)
